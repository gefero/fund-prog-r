...
* Otro ejemplo: al importar datos de un GIS (.shp, .geojson, geodatabaste, etc.) R los interpreta como una lista.
...
# Subsetting/Slicing en R: Listas
* El operador `[[]]` puede usarse con índices calculado. `$` solo puede usarse con nombres
```{r, highlight=TRUE, prompt=TRUE, strip.white=FALSE, collapse=TRUE, tidy=TRUE}
x<-list(foo = 1:4, bar = 0.6, baz = 'hola')
...
* En R es posible utlizar un vector de booleanos (`TRUE`-`FALSE`) como insumo para hacer subsetting/slicing. Tiene que tener la misma longitud que el objeto que vamos a subsetear y la idea es que los miembros del objeto original que están en la "posición" `TRUE` del vector lógico son extraidos para el slice, mientras que los que están en la "posición" `FALSE`, no. 
```{r, highlight=TRUE, prompt=TRUE, strip.white=FALSE, collapse=TRUE, tidy=TRUE}
x<-c("aa", "bb", "cc", "dd", "ee")
...
* Relativamente fácil: `is.na()` devuelve un vector de lógical 
```{r, highlight=TRUE, prompt=TRUE, strip.white=FALSE, collapse=TRUE, tidy=TRUE}
x<-c(1,2,NA,4,NA)
no<-is.na(x)
x1<-x[!no]
...
* Los argumentos pueden ser matcheados por nombre o por posición. Por ejemplo, para la función `sd()` las siguientes son expresiones equivalentes:
```{r, highlight=TRUE, prompt=TRUE, strip.white=FALSE, collapse=TRUE, tidy=TRUE}
data<-rnorm(100)
sd(data)
sd(x=data)
...
sd(na.rm=TRUE, data)
```
* Pese a que es "legal" en R no es recomendable hacer demasiada mezcla con el orden de los argumentos en las funciones

# Funciones en R: matching de argumentos
* Se pueden "mezclar" matching de argumentos por nombre y por posición. Cuando un argumento se llama por nombre se lo saca de la lista y el resto de los argumentos se llaman por la posición
```{r, highlight=TRUE, prompt=TRUE, strip.white=FALSE, collapse=TRUE, tidy=TRUE}
args(lm)
```
